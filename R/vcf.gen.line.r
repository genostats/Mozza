...
vcf.gen.line <- function(genotypes, err.prob, dp1, dp2) {
  ...
  bA1p <- A1; bA1p[bA1p == 0] <- 1-errs1; bA1p[bA1p == 1] <- (1/3)*errs1
  bA2p <- A1; bA2p[bA2p == 0] <- (1/3)*errs1; bA2p[bA2p == 1] <- 1-errs1
  bA3p <- (2/3)*errs1
  
  BA1p <- A2; BA1p[BA1p == 0] <- 1-errs1; BA1p[BA1p == 1] <- (1/3)*errs1
  BA2p <- A2; BA2p[BA2p == 0] <- (1/3)*errs1; BA2p[BA2p == 1] <- 1-errs1
  BA3p <- (2/3)*errs1
  ...
  ##phred likelihoods
  pg00 <- (1-errs1)^rd * (errs1/3)^rd2 * (2*errs1/3)^rd3
  pg01 <- (0.5*(1-errs1)+0.5*(errs1/3))^rd * (0.5*(1-errs1)+0.5*(errs1/3))^rd2 * (2*errs1/3)^rd3
  pg11 <- (errs1/3)^rd * (1-errs1)^rd2 * (2*errs1/3)^rd3
  ...
  GQ <- GQ(phtab)
  GQ[GQ>99] <- 99; GQ <- round(GQ, 0)
  
  ## hard called genotypes
  genoN <- GT(phtab)
  genoN <- c("0/0", "0/1", "1/1")[genoN]
  
  ## If no reads or all error reads,  genotype is missing
  no.read <- which(rd + rd2 <= rd3) 
  genoN[no.read] <- "./."
  GQ[no.read] <- "."
  phtab <- round(phtab, 0)
  phtab[no.read,] <- "."      # possibly silently transforming phtab to characters
  
  vcfj <- paste(genoN,                         # GT = hard called genoypes 
                paste(rd, rd2, sep = ","),     # AD = reads A1 / A2
                rd+rd2+rd3,                    # DP = depth
                GQ,                            # GQ = phred scaled genotype quality
                paste(phtab[,1],phtab[,2],phtab[,3], sep = ","),  # PL = phred scaled probs
                sep = ":")
  return(vcfj)
}
