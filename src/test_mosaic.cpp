...
// [[Rcpp::export]]
void essai() {
  mozza::mosaic M(mozza::Autosomes(), 10, 50.0);
  // Rcout << M.chr_len[0] << "\n";
  // Rcout << M.bpoints[0][0] << "\n";
  // Rcout << M.tiles[0][0] << "\n";
  // Rcout << M.tiles.size() << "\n";
...
// [[Rcpp::export]]
void essai2() {
  mozza::mosaic M1(mozza::Autosomes(), 10, 20);
  mozza::mosaic M2(mozza::Autosomes(), 10, 20);
...
// [[Rcpp::export]]
void essai3() {
  mozza::mosaic M1(mozza::Autosomes(), 1);
  mozza::mosaic M2(mozza::Autosomes(), 2);
...
List test_IBD_sharing(int n, double le1 = 100., double le2 = 100.) {
  ...
  for(int i = 0; i < n; i++) {
    mozza::mosaic M1(mozza::Autosomes(), 100, 20);
    mozza::mosaic M2(mozza::Autosomes(), 100, 20);
    // mozza::mosaic M1(mozza::Autosomes(), 1);
    // mozza::mosaic M2(mozza::Autosomes(), 2);
    mozza::mosaic M(M1, M2, le1, le2);
    R1.push_back( mozza::IBD_sharing(M,M1)  / mozza::lengthAutosomes());
    R2.push_back( mozza::IBD_sharing(M,M2)  / mozza::lengthAutosomes());
    R3.push_back( mozza::IBD_sharing(M1,M2) / mozza::lengthAutosomes());    
  }
  List L;
  L["S1"] = R1;
  L["S2"] = R2;
...
List test_IBD_sibs(int n, int n_haps = 100, double length_tiles = 20.) {
  ...
  for(int i = 0; i < n; i++) {
    ...
    mozza::mosaic M2(mozza::Autosomes(), n_haps, length_tiles);
    mozza::mosaic P1(mozza::Autosomes(), n_haps, length_tiles);
    mozza::mosaic P2(mozza::Autosomes(), n_haps, length_tiles);
    ...
    R0.push_back( std::get<0>(IBD) / mozza::lengthAutosomes());
    R1.push_back( std::get<1>(IBD) / mozza::lengthAutosomes());
    R2.push_back( std::get<2>(IBD) / mozza::lengthAutosomes());
  }
  List L;
  L["IBD0"] = R0;
  L["IBD1"] = R1;
...
XPtr<matrix4> families_of_4_v0(int N, XPtr<matrix4> Haplos, IntegerVector chr, NumericVector dist) {
  ...
  for(int i = 0; i < N; i++) {
    mozza::mosaic M1(mozza::Autosomes(), n_haps, 20);
    mozza::mosaic M2(mozza::Autosomes(), n_haps, 20);
    mozza::mosaic P1(mozza::Autosomes(), n_haps, 20);
    mozza::mosaic P2(mozza::Autosomes(), n_haps, 20);
    mozza::mosaic Ma1(P1, P2, 100, 100);
    mozza::mosaic Ma2(M1, M2, 100, 100);
    mozza::mosaic Mb1(P1, P2, 100, 100);
    mozza::mosaic Mb2(M1, M2, 100, 100);
    ...
    /* if(i == 0) {
      SHOW(mozza::IBD_sharing(M1, M2) / mozza::lengthAutosomes());
      SHOW(mozza::IBD_sharing(P1, P2) / mozza::lengthAutosomes());
      SHOW(mozza::IBD_sharing(P1, Ma1) / mozza::lengthAutosomes());
      SHOW(mozza::IBD_sharing(Ma1,Mb1&) / mozza::lengthAutosomes());
    }*/
  }
  mozza::mappedBed<IntegerVector, NumericVector> MB(Haplos, chr, dist);
  return zygote_to_bed_matrix(x, MB);
...
XPtr<matrix4> families_of_4(int N, XPtr<matrix4> Haplos, IntegerVector chr, NumericVector dist) {
  ...
  for(int i = 0; i < N; i++) {
    mozza::zygote M(mozza::Autosomes(), n_haps, 20);
    mozza::zygote F(mozza::Autosomes(), n_haps, 20);
    x.push_back(M);
    x.push_back(F);
    x.push_back(M+F);
    x.push_back(M+F);
